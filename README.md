...
7. Quand tu (ou Streamlit) appelles `GET /performance?period=ALL` :
   ...
   - Il calcule le P&L % et le drawdown maximum par rapport au pic
   - Streamlit affiche deux graphiques : courbe P&L % et drawdown
...
positions:
  - ticker: "IWDA.AS"       # Ticker Yahoo Finance
    ...
    purchase_date: "2025-06-15"  # Date d'achat (pour le graphique de performance)
```
...
### Exemple de reponse `/sectors`

```json
{
  "sectors": [
    {"name": "Technology", "weight_pct": 25.01},
    {"name": "Financial Services", "weight_pct": 16.43},
    {"name": "Healthcare", "weight_pct": 11.22}
  ],
  "meta": {
    "etfs_analyzed": ["IWDA.AS", "EIMI.MI"],
    "etfs_no_data": ["PANX.PA"],
    "portfolio_coverage_pct": 82.15
  }
}
```

### Exemple de reponse `/performance`
...
```json
{
  ...
  "daily": [
    {
      "date": "2026-02-11",
      "portfolio_value_eur": 4317.01,
      "cost_basis_eur": 4317.01,
      "pnl_eur": 0.0,
      "pnl_pct": 0.0,
      "drawdown_pct": 0.0
    }
